...
import {
  ...
  getLyric,
  getHotComment,
} from "../../../services/musicPlayBar"
import { parseLyric } from "../../../utils/parseLyric"

export const useSongDetailAction = () => {
  const dispatch = useDispatch()
  const getSongDetailAction = useCallback(
    async (ids) => {
...
export const useLyricAction = () => {
  const dispatch = useDispatch()
  const getLyricAction = useCallback(
    async (id) => {
...
export const useHotCommentAction = () => {
  const dispatch = useDispatch()
  const getHotCommentAction = useCallback(
    async (id, type) => {
      const hotComment = await getHotComment(id, type).then(
        (res) => res.hotComments
      )
      dispatch({
